#include "itkBackwardTransformMeshCommand.h"
...
namespace itk
{
  ...
  BackwardTransformMeshCommand::BackwardTransformMeshCommand()
  {
    m_ShortDescription = "Transform a tensor image or mesh from Prolate Spheroidal Coordinates";
  ...
  int BackwardTransformMeshCommand::Execute (int narg, const char* arg[])
  {
    ...
    transformer->SetTransform (transform_inverse);
    transformer->Update();
    
    std::cout<<"Warping..."<<std::endl;
    warper->SetInput (transformer->GetOutput());
    warper->SetDisplacementField (inversedisplacementfield);
    warper->SetInverseDisplacementField (displacementfield);
    warper->Update();
    
    MeshType::Pointer output = warper->GetOutput();
