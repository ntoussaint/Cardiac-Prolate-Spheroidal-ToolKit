...
Module:    $Id: itkNewUoaOptimizer.cxx 1 2010-05-21 14:00:33Z nt08 $
Language:  C++
Author:    $Author: nt08 $
...
=========================================================================*/
#ifndef _itkNewUoaOptimizer_cxx
#define _itkNewUoaOptimizer_cxx
...
namespace itk
{
  ...
  ::NewUoaOptimizer()
  {
    m_MaxFunctionCalls = 5000;
    m_NbInterp = 0;
    m_BestValue = 0;
    m_RhoBeg = 0;
    m_RhoEnd = 0;
    m_Verbosity = 0;
    m_Bounds = new ParameterVectorType[2];
  }
  ...
  ::StartOptimization()
  {
    ...
    unsigned int iprint = m_Verbosity;

    /*     MAXFUN must be set to an upper bound on the number of calls of CALFUN. */
    //  long int max_function_calls = 5000;
    long int max_function_calls = m_MaxFunctionCalls;

    /*     N must be set to the number of variables and must be at least two. */
    if ( m_SpaceDimension == 0 ) 
      return;
    long int n = m_SpaceDimension;

    /*     NPT is the number of interpolation conditions. Its value must be in the  interval [N+2,(N+1)(N+2)/2]. */
    if ( m_NbInterp == 0 )
      m_NbInterp = (2*n)+1;
    long int npt = m_NbInterp;
    ...
    double *w; 
    w = new double[(((npt+13)*(npt+n)+3*(n)*(n+3)/2) + 10)];
    ...
    x = new double[n]; 
    for (int i= 0; i<n; i++)
      x[i] = this->GetInitialPosition()[i] * this->GetScales()[i];
    ...
    if ( m_RhoBeg == 0.0 || m_RhoEnd == 0.0 )
    {
      m_RhoBeg = 0.2;
      m_RhoEnd = 1e-6;
    }
    double rhobeg = m_RhoBeg;
    double rhoend = m_RhoEnd;
    
    if ( ( m_Bounds[0].GetSize() == 0 ) &&
	 ( m_Bounds[1].GetSize() == 0 ) )
    {
      m_Bounds[0].SetSize (m_SpaceDimension, true);
      m_Bounds[1].SetSize (m_SpaceDimension, true);
    ...
    if ( ( m_Bounds[0].GetSize() != m_SpaceDimension ) ||
	 ( m_Bounds[1].GetSize() != m_SpaceDimension ) )
    {
      itkExceptionMacro ("The bounds do not match the space dimensions.");
      return;
    }

    this->newuoa(w, &n, &npt, x, &rhobeg, &rhoend, &iprint, &max_function_calls);
  ...
  int
  NewUoaOptimizer
  ::newuoa(double *w, long int *n, long int *npt, double *x, 
	   double *rhobeg, double *rhoend, unsigned int *iprint, long int * maxfun)
  {
    ...
    static long int id, np, iw, igq, ihq, ixb, ifv, ipq, ivl, ixn, ixo, ixp, 
      ndim, nptm, ibmat, izmat; // i; //-FD Apparemment i est inutilisé
    ...
    this->newuob(n, npt, &x[1], rhobeg, rhoend, iprint, maxfun, &w[ixb], &w[ixo], &w[ixn], &w[ixp], 
		 &w[ifv], &w[igq], &w[ihq], &w[ipq], &w[ibmat], &w[izmat], &ndim, &w[id], &w[ivl], &w[iw]);
    // 		L20:
    /* Free the work space */
    // 		
    // 		L21:
  ...
  int
  NewUoaOptimizer
  ::newuob(long int *n, long int *npt, double *x, 
	   double *rhobeg, double *rhoend, unsigned int *iprint, long int * maxfun, 
	   ...
	   double *d__, double *vlag, double *w)
  {
    ...
    /* Local variables */
    static double f;
    static long int i__, j, k, ih, nf, nh, ip, jp;
    //  static  long int size, size2, bsize; 
    static double dx;
    ...
    NewUoaOptimizer::ParametersType xCoord( m_SpaceDimension );
  ...
  L50:
    ...
    for (unsigned int i=0; i<m_SpaceDimension; i++)
    {
      xCoord[i] = x[i+1] / this->GetScales()[i];
    }
    for (unsigned int i=0; i<m_SpaceDimension; i++)
    {
      xCoord[i] = std::max ( xCoord[i], m_Bounds[0][i] );
      xCoord[i] = std::min ( xCoord[i], m_Bounds[1][i] );
    }    
    for (unsigned int i=0; i<m_SpaceDimension; i++)
    {
      x[i+1] = xCoord[i] * this->GetScales()[i];
    }
    ...
    // 			if (*iprint >= 1) 
    // 			{
    // 				std::cerr<<"\n\r\n\r    At the return from NEWUOA     Number of function values = "<<nf;    
    // 				std::cerr<<" \n\r    Least Value of F = "<<fopt<<"\n\r    The corresponding X is: ";
    // 				for(i__ = 1; i__ <=*n;++i__)
    // 				std::cerr<<"\n\r                              "<<x[i__];
    // 				std::cerr<<"\n\r";
    // 			}
  ...
  L100:
    ...
    this->trsapp(n, npt, &xopt[1], &xpt[xpt_offset], &gq[1], &hq[1], &pq[1], &delta, &d__[1], &w[1], &w[np], &w[np + *n], &w[np + (*n << 1)], &crvmin);
  ...
  L120:
    ...
    if (knew > 0) {
      ...
      this->biglag(n, npt, &xopt[1], &xpt[xpt_offset], &bmat[bmat_offset], &zmat[zmat_offset], &idz, ndim, &knew, &dstep, &d__[1], &alpha, &vlag[1], &vlag[*npt + 1], &w[1], &w[np], &w[np + *n]);
    ...
    if (knew > 0) {
      ...
      if (abs(temp) <= .8) {
	this->bigden(n, npt, &xopt[1], &xpt[xpt_offset], &bmat[bmat_offset], &zmat[zmat_offset], &idz, ndim, &kopt, &knew, &d__[1], &w[1], &vlag[1], &beta, &xnew[1], &w[*ndim + 1], &w[*ndim * 6 + 1]);
      }
    }
  ...
  L310:
    ...
    for (unsigned int i=0; i<m_SpaceDimension; i++)
      std::cout<<xCoord[i]<<" ";
    std::cout<<std::endl;
    f = (double) this->m_CostFunction->GetValue(xCoord);
	...
	e_wsfe();*/	
      std::cout << "\n    Function number = " << nf << "      F = " << f << std::endl;
  ...
  L410:
    this->update(n, npt, &bmat[bmat_offset], &zmat[zmat_offset], &idz, ndim, &vlag[1], &beta, &knew, &w[1]);
  ...
  int 
  NewUoaOptimizer
  ::biglag(long int *n, long int *npt, double *xopt, 
	   ...
	   double *s, double *w)
  {
    ...
    /* Builtin functions */
    //     double atan(double), sqrt(double), cos(double), sin(  double);
  ...
  int
  NewUoaOptimizer
  ::bigden(long int *n, long int *npt, double *xopt, 
	   double *xpt, double *bmat, double *zmat, long int *idz, 
	   ...
	   double *wvec, double *prod)
  {
    ...
    static double ssden, dtest, quart, xoptd, twopi, xopts, denold, denmax, densav, dstemp, sumold, sstemp, xoptsq;
  ...
  int
  NewUoaOptimizer
  ::update(long int *n, long int *npt, double *bmat, 
  ...
  int
  NewUoaOptimizer
  ::trsapp(long int *n, long int *npt, double *xopt, 
